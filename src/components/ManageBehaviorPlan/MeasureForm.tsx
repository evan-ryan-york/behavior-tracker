...
function MeasureForm({ behaviorPlanStage, setBehaviorPlanStage }: Props) {
  ...
  const handleFrequencyDenominatorChnage = (event: SelectChangeEvent) => {
    setPlanForm((pV) => ({ ...pV, frequencyDenominator: event.target.value as "Day" | "Hour" }));
  };
  return (
    <>
      {behaviorsObj && selectedStudent && behaviorPlanStage > 6 && (
        <>
          ...
          <Select onChange={handleMethodChange} value={planForm.measureMethod} sx={{ mt: 1 }}>
          ...
          {planForm.measureMethod === "Frequency" && (
            <>
              ...
              <Box
                sx={{ display: "flex", alignItems: "center", flexWrap: "wrap", gap: "8px", mt: 2 }}
              >
                <Typography>
