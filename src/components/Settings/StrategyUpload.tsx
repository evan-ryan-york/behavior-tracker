...
const StrategyUpload = () => {
  ...
  useEffect(() => {
    ...
    setSuccessMessage(`${uploadsCount} Strategies Uploaded`);
    setLoading(false);
  ...
  const parseData = async (value: Column, index: number) => {
    if (!organization || !loggedInStaff) return;
    ...
    setLoading(true);
    await addDoc({ col: "strategies", data: strategy });
    setUploadsCount((pV) => {
      return pV + 1;
    });
    setLoading(false);
